...
#         # 1. 拇指判断 
#         # 在镜像模式(cv2.flip)下：
#         # - 右手(显示为Right): 拇指通常在手掌的"外侧" (X坐标更大) -> 指尖x < 指关节x
#         # - 左手(显示为Left):  拇指通常在手掌的"外侧" (X坐标更小) -> 指尖x > 指关节x
...
#         # 2. 其他四指判断 (垂直方向，不受左右手影响)
#         for id in range(1, 5):
#             # Y轴向下为正，指尖在上面(y小)说明伸出
#             if lm_list[self.tip_ids[id]]['y'] < lm_list[self.tip_ids[id] - 2]['y']:
#                 fingers.append(1)
#             else:
#                 fingers.append(0)
...
#     # 稍微调整大小方便查看，但保留原比例更好，这里简单resize
